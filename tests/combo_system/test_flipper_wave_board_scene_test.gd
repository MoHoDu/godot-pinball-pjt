...
func _run() -> void:
	...
	flipper.emit_signal(&"rotation_sweep_resolved", active)
	_expect(combo.combo_count == 1, "Active flipper hits must not increase the combo count.")
	_expect(is_equal_approx(combo.time_remaining, 3.0), "The first active flipper hit after a scoring hit should refresh the timer.")
	combo.advance_time(1.0)
	active.body_entered.emit(wall)
	_expect(is_equal_approx(combo.time_remaining, 2.0), "Dynamic ball wall contact must not refresh the combo timer.")
	...
	combo.register_hit(1.0)
	flipper.emit_signal(&"rotation_sweep_resolved", second_ball)
	_expect(combo.combo_count == 1, "Second dynamic ball flipper hit should preserve the scoring combo count.")
