...
func _run() -> void:
	...
	# 보스 검수 씬에도 보스 루프가 물려 있다
	var boss_packed := load("res://scenes/test_sfx/boss_art_sfx_test.tscn") as PackedScene
	if _expect(boss_packed != null, "보스 검수 씬이 로딩된다"):
		...
		_expect(boss_bgm != null and boss_bgm.playing, "보스 씬에서 보스 루프가 흐른다")
