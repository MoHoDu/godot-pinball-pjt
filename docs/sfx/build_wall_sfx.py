...
# 벽 전용 유리 팅. 주파수는 공 충돌음과 같게 두되 **감쇠만 짧게** 한다.
# 파일럿의 GLASS_MODES_HIT 는 tau 가 0.018~0.033 으로 목재의 3배라,
# 게인을 0.17 로 낮춰도 고속 충돌 에너지의 64% 를 유리가 차지했다(실측).
# 가이드가 요구하는 것은 "작은 유리 팅"이지 유리가 주인공인 소리가 아니다.
...
PROFILES = {
    "base": {
        "low": (LOW_GATE, 0.80, 0.18, 0.00, 0.52, 0.08, 0.26, 0),
        "mid": (MID_GATE, 0.64, 0.62, 0.00, 0.64, 0.12, 0.20, 1),
        "high": (HIGH_GATE, 0.58, 0.40, 0.30, 0.70, 0.18, 0.20, 2),
    },
    # 한 단계 더. 여기서 더 가면 p.61 의 "강한 저음 충격"으로 넘어간다.
    "thicker": {
        "low": (LOW_GATE, 0.72, 0.34, 0.00, 0.78, 0.06, 0.34, 0),
        "mid": (MID_GATE, 0.56, 0.86, 0.00, 0.92, 0.09, 0.28, 1),
        "high": (HIGH_GATE, 0.52, 0.60, 0.26, 0.98, 0.14, 0.28, 2),
    },
    "sharp": {
        "low": (LOW_GATE, 0.92, 0.00, 0.00, 0.12, 0.30, 0.10, 0),
        "mid": (MID_GATE, 0.80, 0.30, 0.00, 0.16, 0.40, 0.07, 1),
        "high": (HIGH_GATE, 0.70, 0.16, 0.44, 0.20, 0.52, 0.07, 2),
    },
}
