...
func _resolve_swept_ball(
	...
	resolved_velocity = _ensure_minimum_active_hit_velocity(
		resolved_velocity,
		surface_velocity
	)
...
## 활성 타격의 반사·구역·패링 계산이 끝난 방향을 유지하면서 공용 최소 출사
## 속력을 보장합니다. 결과 방향이 사라진 경우에만 표면 이동 방향을 사용합니다.
func _ensure_minimum_active_hit_velocity(
	resolved_velocity: Vector2,
	surface_velocity: Vector2
) -> Vector2:
	var minimum_speed := float(
		state_rules.get(&"minimum_active_hit_speed")
	)
	var resolved_speed := resolved_velocity.length()
	if minimum_speed <= 0.0 or resolved_speed >= minimum_speed:
		return resolved_velocity
	if not resolved_velocity.is_zero_approx():
		return resolved_velocity.normalized() * minimum_speed
	if surface_velocity.is_zero_approx():
		return resolved_velocity
	return surface_velocity.normalized() * minimum_speed
