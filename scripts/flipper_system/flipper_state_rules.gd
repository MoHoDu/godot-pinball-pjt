...
var _minimum_active_hit_speed: float = 320.0
...
## 활성 플리퍼가 실제로 타격한 공에 보장하는 최소 출사 속력입니다.
@export_range(0.0, 5000.0, 1.0, "suffix:px/s")
var minimum_active_hit_speed: float:
	get:
		return _minimum_active_hit_speed
	set(value):
		_minimum_active_hit_speed = clampf(value, 0.0, 5000.0)
		emit_changed()
