...
func _process(delta: float) -> void:
	...
	if trails_alive or smoke_alive or _capture_active:
		queue_redraw()
...
func spawn_launch() -> void:
	...
	for index in shot_rules.smoke_count:
		var spread := deg_to_rad(shot_rules.tick_spread_degrees) * 0.5
		var offset := randf_range(-spread, spread)
		_smoke.append({
			"born": _time,
			"life": shot_rules.trail_lifetime * 1.4,
			"origin": _bumper_center() + direction * radius * 0.55,
			"dir": direction.rotated(offset),
			"length": radius * randf_range(0.18, 0.34),
			"radius": radius * shot_rules.chip_radius_ratio * randf_range(1.1, 1.8),
		})
...
func _draw() -> void:
		...
		for anchor in _shot_bumper.get_launch_anchors():
			...
			if not is_selected:
				...
				if shot_rules.unselected_path_alpha <= 0.0:
					continue
			draw_line(world * radius * 0.6, world * length, color, width, true)
	...
	for item in _trails:
		...
		draw_line(start, end, color, shot_rules.selected_path_width * (1.4 - t), true)
	...
	for item in _smoke:
		...
		draw_circle(position, float(item["radius"]) * (0.7 + 0.6 * t), color)
