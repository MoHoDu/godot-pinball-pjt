...
##
## top_level을 켜서 부모 변환을 아예 상속하지 않습니다.
## 자식으로 두고 global_rotation만 덮어쓰면, 우리가 쓴 뒤에 물리 서버가 바디를
## 한 스텝 더 돌리기 때문에 회전 속도에 비례한 오차가 남습니다.
## (실측: 바디 3rad/s에서 한 스텝치인 0.05rad가 계속 남았다)
...
## 우리가 만들어 낸 시선 각도입니다.
## global_rotation을 읽어서 보간하면 부모 바디의 회전이 섞여 들어와 시선이 뒤처집니다.
...
func _ready() -> void:
	...
	global_rotation = _current_angle
...
func _physics_process(delta: float) -> void:
	...
	# top_level이라 위치를 상속하지 않으므로 직접 따라갑니다.
	...
	# ★ global_rotation이 아니라 _current_angle을 보간합니다.
	# global_rotation을 읽어서 보간하면 매 프레임 부모 바디가 돌려놓은 만큼이
	# 오차로 섞여 들어옵니다. 그러면 시선이 회전 속도에 비례해 뒤처집니다.
	# (실측: 바디 3rad/s + sharpness 18에서 0.19rad 고정 오차)
