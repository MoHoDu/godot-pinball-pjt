...
이 씬은 코인 필드, 종료 유예 환산, 공용 지갑 연결, 임시 코인 HUD를 함께 제공한다.
보상 플레이스홀더에는 현재 공용 지갑 잔액이 표시된다. 실제 보상 씬도
...
세 상속 웨이브 모두 `stage_wave_coin_system.tscn`이 연결되어 웨이브마다 코인 12개를
생성하고, 획득 코인을 다음 보상·웨이브까지 누적한다.
